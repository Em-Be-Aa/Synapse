...
TextWidget::TextWidget(Font* font, std::string initialText, glm::vec4 Color, glm::vec2 Size, glm::vec2 Position)
    : Widget(Size, Position), fontRef(font), text(initialText)
{
    widgetColor = Color;
}
...
void TextWidget::Update(double dT)
{
    ...
    float cursorX = normalizedPos.x * windowWidth;
    float cursorY = normalizedPos.y * windowHeight + fontRef->GetPixelHeight();
    ...
    for (char c : text)
    {
        ...
        glm::vec2 uvOffset = { quad.s0, quad.t0 };
