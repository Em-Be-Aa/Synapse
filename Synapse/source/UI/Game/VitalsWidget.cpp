...
void VitalsWidget::Init()
{
	...
	Momentum->GetText()->SetVisibility(true);
	...
	Focus->GetText()->SetVisibility(true);
	Focus->SetZOrder(1);
	...
	Recovery->GetText()->SetVisibility(true);
	...
	Insight->GetText()->SetVisibility(true);
	Insight->SetZOrder(1);
...
void VitalsWidget::Update(double dT)
{
	...
	Momentum->GetText()->SetText("+" + std::to_string(static_cast<int>(std::round(VC.GetMomentum()))) + "%");
	Recovery->GetText()->SetText("+" + std::to_string(static_cast<int>(std::round(VC.GetRecovery() * 100.0f))) + "%");
	Focus->GetText()->SetText("+" + std::to_string(static_cast<int>(std::round(VC.GetFocus() * 100.0f))) + "%");
	Insight->GetText()->SetText("+" + std::to_string(static_cast<int>(std::round(VC.GetInsight() * 100.0f))) + "%");
